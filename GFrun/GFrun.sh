...
F_Git(){
	...
	if [ -f $HOME/GFrunLocal/GFrun/GFrun/GFrun.sh ]; then
		...
		cd $HOME && wget -N https://codeload.github.com/xonel/GFrun/zip/$Vbranche.zip 1>/dev/null && unzip -o $Vbranche.zip 1>/dev/null && mv $HGFrun-$Vbranche $HGFrun 1>/dev/null
...
F_Update(){
	...
	read -p 'Do you want update CORE $Vbranche (N/y) ?' Vo
			case "$Vo" in
				 y|Y)		
					cd $HOME && wget -N https://codeload.github.com/xonel/GFrun/zip/$Vbranche.zip 1>/dev/null && unzip -o $Vbranche.zip 1>/dev/null && mv $HGFrun-$Vbranche $HGFrun 1>/dev/null
					echo `color 36 "<<< wget https://codeload.github.com/xonel/GFrun/zip/$Vbranche.zip"`
